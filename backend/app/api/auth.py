...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
    ...
    try:
        ...
        # Increased timeout to 30 seconds and add retry logic
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if response.status_code != 200:
                        # Non-200 response - don't retry, this is an auth error
                    ...
                    if attempt < max_retries - 1:
                        # Retry on timeout
                        logger.warning(f"Token verification timeout, retrying... (attempt {attempt + 1}/{max_retries})")
                        await asyncio.sleep(1)  # Wait 1 second before retry
                        continue
                    else:
                        # All retries failed - return 503 (service unavailable) instead of 401
                        logger.error("Token verification timeout after retries")
                    ...
                    last_exception = e
                    if attempt < max_retries - 1:
                        logger.warning(f"Token verification error, retrying... (attempt {attempt + 1}/{max_retries}): {e}")
                        await asyncio.sleep(1)
                        continue
                    else:
                        logger.error(f"Token verification failed after retries: {e}", exc_info=True)
                        raise HTTPException(status_code=503, detail="Service temporarily unavailable. Please try again.")
